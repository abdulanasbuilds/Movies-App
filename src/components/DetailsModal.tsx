...
import { X, Play, Download, Star, Clock, Calendar, Settings as SettingsIcon } from 'lucide-react';
...
export const DetailsModal: React.FC<DetailsModalProps> = ({ item, onClose }) => {
  ...
  useEffect(() => {
    if (item) {
      setStreamUrl(streamResolver.getStreamUrl(item.id, item.type, currentEpisode, selectedSeason));
    }
  }, [item, currentEpisode, selectedSeason]);
  ...
  const handleNextEpisode = () => {
    if (seasonDetails && currentEpisode < seasonDetails.episodes.length) {
  ...
  const hasNextEpisode = !!(
    ...
    (tvDetails && selectedSeason < tvDetails.number_of_seasons)
  );
        ...
        >
          {isPlaying ? (
            <div className="p-4 md:p-8 space-y-4">
              <MediaPlayer 
                url={streamUrl}
                title={`${item.title} - ${item.isTV ? `S${selectedSeason}E${currentEpisode}` : 'Movie'}`}
                onClose={() => setIsPlaying(false)}
                onEnded={handleNextEpisode}
                hasNextEpisode={hasNextEpisode}
              />
              <div className="flex items-center justify-between">
                 <p className="text-gray-400 text-sm">Currently playing from: <span className="text-primary truncate max-w-xs inline-block align-bottom ml-1">{streamUrl}</span></p>
                 <button 
              ...
              {showOverride && (
                <div className="p-4 bg-white/5 rounded-2xl border border-white/10 flex space-x-2">
                  <input 
                    type="text"
                    placeholder="Enter stream URL (.mp4, .m3u8, or iframe link)..."
                    className="flex-1 bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-sm outline-none focus:ring-1 focus:ring-primary"
                    value={streamUrl}
                    onChange={(e) => setStreamUrl(e.target.value)}
                  />
                  <button 
                    onClick={() => {
                      streamResolver.setOverride(item.id, streamUrl);
                ...
                <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <button 
                      ...
                      className="flex-1 flex items-center justify-center px-8 py-4 bg-primary text-white rounded-2xl font-bold hover:bg-primary/90 transition-all active:scale-95"
                    >
                  ...
                  <div className="pt-8 border-t border-white/5 space-y-6">
                    ...
                    <div className="space-y-3">
                      {isLoadingSeason ? (
                        <div className="space-y-3">
                          {[1,2,3].map((i: number) => <div key={i} className="h-20 bg-white/5 animate-pulse rounded-2xl" />)}
                        </div>
